...
void printTransform(const char* name, const tf::Transform t) {
	ROS_DEBUG_STREAM(
			name << ": Translation " << t.getOrigin().x() << " "
...
GraphManager::GraphManager(ros::NodeHandle nh, GLViewer* glviewer) :
			...
			reset_request_(false), last_batch_update_(std::clock()),
			marker_id(0), last_matching_node_(-1),
  ...
  pose_pub = nh.advertise<geometry_msgs::PoseStamped>("/stripped_rgbdslam_1/currentPose", 100);
	...
	timer_ = nh.createTimer(ros::Duration(0.1),
			&GraphManager::broadcastTransform, this);

	ransac_marker_pub_ = nh.advertise<visualization_msgs::Marker> (
			"/rgbdslam/correspondence_marker", 10);
	aggregate_cloud_pub_ = nh.advertise<sensor_msgs::PointCloud2> (
			"/rgbdslam/aggregate_clouds", 20);
...
void GraphManager::drawFeatureFlow(cv::Mat& canvas, cv::Scalar line_color,
	...
	std::clock_t starttime = std::clock();
	ROS_DEBUG("Number of features to draw: %d",
			(int) last_inlier_matches_.size());

	const double pi_fourth = 3.14159265358979323846 / 4.0;
	const int line_thickness = 1;
	const int circle_radius = 6;
	if (graph_.size() < 2)
		return;// || last_matching_node_ < 0) return; //feature flow is only available between at least two nodes
	...
	cv::drawKeypoints(tmpimage, newernode->feature_locations_2d_, tmpimage,
			cv::Scalar(90), 5);
	canvas += tmpimage;
	for (unsigned int mtch = 0; mtch < last_inlier_matches_.size(); mtch++) {
		...
		if (hypotenuse > 1.5) { //only larger motions larger than one pix get an arrow tip
			...
			cv::circle(canvas, p, circle_radius - 2, circle_color,
					line_thickness, 8);
		}
	}
	ROS_INFO_STREAM_COND_NAMED(
...
std::vector<int> GraphManager::getPotentialEdgeTargets(const Node* new_node,
		int max_targets) {
...
bool GraphManager::addNode(Node* new_node) {
	...
	/*
	ROS_ERROR("hallo welt1");
	ofstream file;
	file.open("/home/rimesime/Desktop/name.txt");
	file << "hallo welt1" << endl;
	file.close();
	ROS_ERROR("hallo welt1");
 	 */
	...
	if (new_node->feature_locations_2d_.size() <= 50) {
		ROS_DEBUG("found only %i features on image, node is not included",
				(int) new_node->feature_locations_2d_.size());
		return false;
	}
	...
	ROS_DEBUG("New Node with id %i has address %p (GraphManager is %p)",
			new_node->id_, new_node, this);
	...
	if (graph_.size() == 0) {
		...
		Q_EMIT setGUIInfo(
				message.sprintf(
						"Added first node with %i keypoints to the graph",
						(int) new_node->feature_locations_2d_.size()));
	...
	ROS_INFO("Comparing new node (%i) with previous node %i / %i",
			new_node->id_, (int) graph_.size() - 1, prev_frame->id_);
	...
	if (mr.edge.id1 >= 0 && !isBigTrafo(mr.edge.mean)) {
		...
		if (addEdgeToHogman(mr.edge, true)) {
			ROS_INFO("Added Edge between %i and %i. Inliers: %i", mr.edge.id1,
					mr.edge.id2, (int) mr.inlier_matches.size());
	...
	std::vector<int> vertices_to_comp = getPotentialEdgeTargets(new_node,
			global_connectivity); //vernetzungsgrad
...
#define QT_NO_CONCURRENT
	...
	for (int i = 0; i < results.size(); i++) {
		...
		if (mr.edge.id1 >= 0) {
			if (addEdgeToHogman(mr.edge, isBigTrafo(mr.edge.mean))) { //TODO: result isBigTrafo is not considered
				ROS_INFO("Added Edge between %i and %i. Inliers: %i",
						mr.edge.id1, mr.edge.id2,
						(int) mr.inlier_matches.size());
				last_matching_node_ = mr.edge.id1;
				last_inlier_matches_ = mr.inlier_matches;
				last_matches_ = mr.all_matches;
			}
	...
	if (optimizer_->edges().size() > num_edges_before) { //Success
		...
		pointcloud_type const * the_pc(&(new_node->pc_col));
		Q_EMIT setPointCloud(the_pc, latest_transform_);
		ROS_DEBUG("GraphManager is thread %d",
	...
	Q_EMIT
			setGUIInfo(
					message.sprintf(
							"%s, Graph Size: %iN/%iE, Duration: %f, Inliers: %i, &chi;<sup>2</sup>: %f",
							(optimizer_->edges().size() > num_edges_before) ? "Added"
									: "Ignored",
							(int) optimizer_->vertices().size(),
...
void GraphManager::mat2RPY(const Eigen::Matrix4f& t, double& roll,
...
bool GraphManager::isBigTrafo(const Eigen::Matrix4f& t) {
	...
	return (dist > global_min_translation_meter || max_angle
			> global_min_rotation_degree);
}
...
bool GraphManager::isBigTrafo(const Transformation3& t) {
	...
	infostring.sprintf("Rotation:% 4.2f, Distance: % 4.3fm", angle_around_axis,
			dist);
	Q_EMIT setGUIInfo2(infostring);
	return (dist > global_min_translation_meter || angle_around_axis
			> global_min_rotation_degree);
}
void GraphManager::visualizeFeatureFlow3D(unsigned int marker_id,
	...
	if (ransac_marker_pub_.getNumSubscribers() > 0) { //don't visualize, if nobody's looking
		...
		AISNavigation::PoseGraph3D::VertexIDMap v_idmap =
				optimizer_->vertices();
		// end of initialization
		ROS_DEBUG("Matches Visualization start: %lu Matches, %lu Inliers",
				last_matches_.size(), last_inlier_matches_.size());
		...
		if (draw_outlier) {
			for (unsigned int i = 0; i < last_matches_.size(); i++) {
				...
				marker_lines.points.push_back(
						pointInWorldFrame(
								last->feature_locations_3d_[newer_id],
								newer_v->transformation));
				Node* prev = graph_.find(last_matching_node_)->second;
				marker_lines.points.push_back(
						pointInWorldFrame(
								prev->feature_locations_3d_[earlier_id],
		...
		for (unsigned int i = 0; i < last_inlier_matches_.size(); i++) {
			...
			Node* last = graph_.find(graph_.size() - 1)->second;
			marker_lines.points.push_back(
					pointInWorldFrame(last->feature_locations_3d_[newer_id],
							newer_v->transformation));
			Node* prev = graph_.find(last_matching_node_)->second;
			marker_lines.points.push_back(
					pointInWorldFrame(prev->feature_locations_3d_[earlier_id],
							earlier_v->transformation));
		}

		ransac_marker_pub_.publish(marker_lines);
		ROS_DEBUG_STREAM(
				"Published  " << marker_lines.points.size() / 2 << " lines");
	}
	ROS_INFO_STREAM_COND_NAMED(
			((std::clock() - starttime) / (double) CLOCKS_PER_SEC)
					> global_min_time_reported,
...
QList<QPair<int, int> >* GraphManager::getGraphEdges() const {
	...
	AISNavigation::PoseGraph3D::EdgeSet::iterator edge_iter =
			optimizer_->edges().begin();
	for (; edge_iter != optimizer_->edges().end(); edge_iter++) {
		v1
				= static_cast<AISNavigation::PoseGraph3D::Vertex*> ((*edge_iter)->from());
		v2
				= static_cast<AISNavigation::PoseGraph3D::Vertex*> ((*edge_iter)->to());
...
void GraphManager::visualizeGraphNodes() const {
	...
	if (marker_pub_.getNumSubscribers() > 0) { //don't visualize, if nobody's looking
		...
		int counter = 0;
		AISNavigation::PoseGraph3D::Vertex* v; //used in loop
		AISNavigation::PoseGraph3D::VertexIDMap::iterator vertex_iter =
				optimizer_->vertices().begin();
		for (/*see above*/; vertex_iter != optimizer_->vertices().end(); vertex_iter++, counter++) {
			v
					= static_cast<AISNavigation::PoseGraph3D::Vertex*> ((*vertex_iter).second);
...
bool GraphManager::addEdgeToHogman(AIS::LoadedEdge3D edge, bool largeEdge) {
	...
	if (!v1 || !v2) {
		if (!largeEdge) {
			ROS_INFO(
					"Edge to new vertex is to short, vertex will not be inserted");
			return false;
		}
	}

	if (!v1) {
		v1 = optimizer_->addVertex(edge.id1, Transformation3(),
				Matrix6::eye(1.0));
		assert(v1);
	}
	if (!v2) {
		v2 = optimizer_->addVertex(edge.id2, Transformation3(),
				Matrix6::eye(1.0));
		assert(v2);
	}
...
void GraphManager::optimizeGraph() {
	...
	ROS_INFO_STREAM(
			"Hogman Statistics: " << optimizer_->vertices().size()
					<< " nodes, " << optimizer_->edges().size() << " edges. "
					<< "chi2: " << optimizer_->chi2() << ", Iterations: "
					<< currentIt);

	freshlyOptimized_ = true;

	AISNavigation::PoseGraph3D::Vertex* v = optimizer_->vertex(
...
void GraphManager::broadcastTransform(const ros::TimerEvent&) {
	...
	br_.sendTransform(
			tf::StampedTransform(world2cam_, time_of_last_transform_,
					"/openni_camera", "/slam_transform"));

	if (!batch_processing_runs_)
		br_.sendTransform(
				tf::StampedTransform(cam2rgb, time_of_last_transform_,
...
void GraphManager::deleteLastFrame() {
	...
	AISNavigation::PoseGraph3D::Vertex* v_to_del = optimizer_->vertex(
			optimizer_->vertices().size() - 1);//last vertex
...
void GraphManager::saveIndividualClouds(QString filename) {
	...
	QFuture<void> f1 = run(this, &GraphManager::saveIndividualCloudsToFile,
			filename);
	//f1.waitForFinished();
}
#else
...
void GraphManager::saveIndividualCloudsToFile(QString file_basename) {
	std::clock_t starttime = std::clock();
	ROS_INFO(
			"Saving all clouds to %sxxxx.pcd, this may take a while as they need to be transformed to a common coordinate frame.",
			qPrintable(file_basename));
	...
	for (unsigned int i = 0; i < optimizer_->vertices().size(); ++i) {
		...
		Eigen::Vector4f sensor_origin(world2cam.getOrigin().x(),
				world2cam.getOrigin().y(), world2cam.getOrigin().z(),
				world2cam.getOrigin().w());
		Eigen::Quaternionf sensor_orientation(world2cam.getRotation().w(),
				world2cam.getRotation().x(), world2cam.getRotation().y(),
				world2cam.getRotation().z());
		...
		Q_EMIT setGUIStatus(
				message.sprintf("Saving to %s: Transformed Node %i/%i",
						qPrintable(filename), i,
						(int) optimizer_->vertices().size()));
...
void GraphManager::saveAllCloudsToFile(QString filename) {
	...
	ROS_INFO(
			"Saving all clouds to %s, this may take a while as they need to be transformed to a common coordinate frame.",
			qPrintable(filename));
	batch_processing_runs_ = true;
	tf::Transform world2cam;
	//fill message
	...
	for (unsigned int i = 0; i < optimizer_->vertices().size(); ++i) {
		...
		cam2rgb.setOrigin(tf::Point(0, -0.04, 0));
		world2cam = cam2rgb * transform;
		transformAndAppendPointCloud(graph_[i]->pc_col, aggregate_cloud,
				world2cam, Max_Depth);
		Q_EMIT setGUIStatus(
				message.sprintf("Saving to %s: Transformed Node %i/%i",
						qPrintable(filename), i,
	...
	Q_EMIT setGUIStatus(
			message.sprintf("Saved %d data points to %s",
					(int) aggregate_cloud.points.size(), qPrintable(filename)));
	ROS_INFO("Saved %d data points to %s", (int) aggregate_cloud.points.size(),
			qPrintable(filename));
...
void GraphManager::sendAllClouds() {
	...
	for (unsigned int i = 0; i < optimizer_->vertices().size(); ++i) {
		...
		br_.sendTransform(
				tf::StampedTransform(world2cam, time_of_transform,
						"/openni_camera", "/batch_transform"));
...
void transformAndAppendPointCloud(const pointcloud_type &cloud_in,
	...
	ROS_INFO("cloud_to_append_to_original_size = %i",
			cloud_to_append_to_original_size);
	...
	for (size_t i = 0; i < cloud_in.points.size(); ++i) {
		...
		Eigen::Map < Eigen::Vector3f
				> p_out(
						&cloud_to_append_to.points[j
								+ cloud_to_append_to_original_size].x, 3, 1);
		if (compact) {
			cloud_to_append_to.points[j + cloud_to_append_to_original_size]
					= cloud_in.points[i];
		}
		//filter out points with a range greater than the given Parameter or do nothing if negativ
		if (Max_Depth >= 0) {
			if (pcl::squaredEuclideanDistance(cloud_in.points[i], origin)
					> Max_Depth * Max_Depth) {
		...
		if (pcl_isnan (cloud_in.points[i].x)
				|| pcl_isnan (cloud_in.points[i].y)
				|| pcl_isnan (cloud_in.points[i].z)) {
			if (!compact)
				j++;
			continue;
		}
